#!/usr/bin/env python3
"""
Environment File Encryption Script for AllBright
===========================================

This script encrypts sensitive .env files for secure storage.
Supports AES-256-GCM encryption with password-based key derivation.

Files to encrypt:
- .env (production secrets)
- .env.example (template)
- backend/.env (backend secrets)
- apps/dashboard/.env (frontend secrets)

Usage:
    python3 encrypt_env_files.py
...
def prompt_for_master_password() -> str:
    """Prompt user for master password for key derivation."""
    import getpass
    while True:
        password = getpass("Enter master password for encryption: ")
...
def main():
    ...
    # Get master password
    password = args.password or prompt_for_master_password()
    ...
    if args.action == 'encrypt':
        ...
        print(f"\n📋 Next steps:")
        print(f"   1. Keep your master password safe")
        print(f"   2. Store the salt file securely: {salt_file}")
        print(f"   3. Delete or move original .env files to .env.bak")
        print(f"   4. Decrypt with: python3 encrypt_env_files.py decrypt --password 'your-password'")
